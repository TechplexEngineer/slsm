...
(function($) {
    ...
    $.validationEngine = {
        ...
        validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
            ...
            if(!$(caller).attr("id")) $.validationEngine.debug("This field does not have an ID attribute( name & class displayed): "+$(caller).attr("name")+" "+$(caller).attr("class"))
            ...
            function _confirm1(caller,rules,position,which){		 // VALIDATE FIELD MATCH
                confirmField = rules[position+1];
                console.log(position + " " + confirmField );

                console.log($(caller).attr('value') + " | " + $("#"+confirmField).attr('value'));
                if($(caller).attr('value') != $("#"+confirmField).attr('value'))
                {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules[which].alertText+"<br />";
                }
            }
        ...
        debug : function(error) {
            if(!$("#debugMode")[0]){
                $("body").append("<div id='debugMode'><div class='debugError'><strong>This is a debug mode, you have a problem with your form, the following info should help, refresh when you think you nailed down the problem</strong></div></div>");
            }
