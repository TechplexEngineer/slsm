...
update()
{
    string action = "?action=register";
    string region = llEscapeURL(llGetRegionName());
    string obj = llEscapeURL(llGetObjectName());
    string args = "&name="+obj+"&region="+region+"&key="+(string)llGetKey()+"&xml="+(string)chan+"&id="+(string)myid+"&time="+(string)llGetUnixTime()+"&pos="+(string)llGetPos()
    httpid = llHTTPRequest(url + args, [], "");
    //llGetKey(), llGetPos,llGetRegion, llGetOwner
...
default
{
    ...
    {
//        open data channel
        if(num_tex != llGetInventoryNumber(INVENTORY_TEXTURE))
            update();
